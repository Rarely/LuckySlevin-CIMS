<?php if (count($notifications) > 0) { ?>
    <?php foreach ($notifications as $notification): ?>
        ...
        <a href="#" class="list-group-item <?php if ($isread == 0) echo 'active' ?>" onclick="Ajax.Notifications.setNotified(this, <?php echo $id ?>); Ajax.Idea.showIdea(<?php echo $ideaid ?>);">
            <h5 class="list-group-item-heading"><?php echo h($notification['Notification']['title']); ?></h5>
            <p class="list-group-item-text"><?php echo h($notification['Notification']['message']); ?></p>
        </a>
    <?php endforeach; ?>
